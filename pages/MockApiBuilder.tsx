...
export const MockApiBuilder: React.FC<MockApiBuilderProps> = ({ onBack, setActions }) => {
  ...
  React.useEffect(() => {
    setActions(
      <button 
        onClick={addRoute}
        className="bg-primary text-white font-black px-4 py-2 rounded-xl shadow-lg shadow-primary/20 flex items-center gap-2 active:scale-95 transition-all text-xs"
      >
        <span className="material-symbols-outlined text-[18px]">add_circle</span> New Route
      </button>
    );
  }, [routes]);
